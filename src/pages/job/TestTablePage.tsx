  ...
  const jsonData: TestTable[] = [
    {
      ...
      orderTotal: 500,
      orderDate: new Date("01/01/2024"),
    },
    {
      emailAddress: "sam@yahoo.com",
      orderTotal: 100,
      orderDate: new Date("02/02/2024"),
    },
    {
      emailAddress: "sahil@yahoo.com",
      orderTotal: 100,
      orderDate: new Date("03/03/2024"),
    },
    {
      emailAddress: "test@yahoo.com",
      orderTotal: 200,
      orderDate: new Date("04/04/2024"),
    },
  ];
  ...
  const getOrderData = (emailAddress: string, monthNumber: number) => {
    const filterData = jsonData.filter(
      (data) =>
        ...
        data.orderDate.getMonth() == monthNumber
    );

    const totalOrder = filterData.reduce(
      (total, data) => total + data.orderTotal,
      0
    );
          ...
          {jsonData.map((rowData, index) => (
            <tr>
              {monthNames.map((colData, index) => (
                <td>{getOrderData(rowData.emailAddress, index)}</td>
              ))}
            </tr>
          ))}
        </tbody>
