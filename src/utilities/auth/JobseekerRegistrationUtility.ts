...
const JobseekerRegistrationUtility = () => {
    ...
    const [jobseekerRegistration, setJobseekerRegistration] = useState<JobseekerRegistrationModel>(initialJobseekerRegistration);
    ...
    const onJobseekerRegistration = async () => {
        ...
        if (isValidate()) {
            ...
            const response = await createJobseekerRegistrationAsync(jobseekerRegistration);
    ...
    const isValidate = () => {
        ...
        if (jobseekerRegistration.confirmPassword !== "" && jobseekerRegistration.password !== "") {
            ...
            // const passwordRegex = /^(?=.*[a-zA-Z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{5,}$/;
             if (!passwordRegex.test(jobseekerRegistration.password)) {
                newErrors.push({
                    fieldName: "password",
                    errorMessage: "Password must be at least 5 characters long, contain at least one letter, one uppercase letter, one number, and one special character",
                });
            }
    ...
    return {
        jobseekerRegistration, onJobseekerRegistration, onTextFieldChange, errorInfo,
