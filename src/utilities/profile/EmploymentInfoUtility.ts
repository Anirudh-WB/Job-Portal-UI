...
const EmploymentInfoUtility = (loginUserId:number) => {
  ...
  const [employmentInfo, setEmploymentInfo] = useState<EmploymentInfoModel>(initialEmploymentInfo);
      ...
      if (response.status ===200){
        if (response.data !== null) {
         // alert(JSON.stringify(response.data));
          ...
          setEmploymentInfo(response.data   ? response.data : initialEmploymentInfo);
    ...
    if (isValidate()) {
      let response;
        if (employmentInfo.id > 0) {
            response = await updateEmploymentInfoAsync(employmentInfo, employmentInfo.id);
        } else {
  ...
  const isValidate = () => {
    ...
    if (employmentInfo.expectedCTC === 0 || employmentInfo.expectedCTC.toString() ==="" ) {
    ...
    if (employmentInfo.currentCTC === 0 || employmentInfo.currentCTC.toString() ==="" ) {
